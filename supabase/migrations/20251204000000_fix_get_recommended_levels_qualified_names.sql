-- Fix get_recommended_levels to use fully qualified table names
-- This is required when using SET search_path = '' for security
-- Reference: https://supabase.com/docs/guides/database/database-linter?lint=0011_function_search_path_mutable

-- ============================================
-- Function: Get recommended levels for student (WITH QUALIFIED NAMES)
-- ============================================
CREATE OR REPLACE FUNCTION public.get_recommended_levels(
  p_student_id UUID,
...
DECLARE
  ...
  v_weak_skills TEXT[];
BEGIN
  -- Get student's proficiency level (QUALIFIED NAME)
  ...
  FROM (
    SELECT skill_name
    FROM public.get_student_weak_areas(p_student_id, 3)
  ) AS weak;
  ...
  WHERE
    -- Exclude completed levels (QUALIFIED NAMES)
    NOT EXISTS (
    ...
    -- Include only levels for student's class (if class restriction exists) (QUALIFIED NAMES)
    AND (
...
COMMENT ON FUNCTION public.get_recommended_levels IS 'Returns personalized level recommendations based on proficiency and weak areas (with qualified table names)';

-- ============================================
-- Function: Get levels filtered by proficiency (WITH QUALIFIED NAMES)
-- ============================================
CREATE OR REPLACE FUNCTION public.get_levels_for_proficiency(
  p_student_id UUID,
...
BEGIN
  ...
  -- Get difficulty range (QUALIFIED NAME)
  SELECT gdr.min_difficulty, gdr.max_difficulty
  INTO v_min_difficulty, v_max_difficulty
  FROM public.get_difficulty_range_for_proficiency(v_proficiency_level) gdr;
  ...
  SELECT
    ...
    -- Mark as recommended if in top 10 recommendations (QUALIFIED NAME)
    EXISTS (
      SELECT 1 FROM public.get_recommended_levels(p_student_id, 10) rec
      WHERE rec.level_id = l.id
    ) AS is_recommended
  FROM public.levels l
  LEFT JOIN public.level_progress lp ON lp.level_id = l.id AND lp.student_id = p_student_id
  WHERE
    ...
    -- Class restriction (QUALIFIED NAME)
    AND (
      l.allowed_classes IS NULL
      OR l.allowed_classes = ARRAY[]::TEXT[]
      OR EXISTS (
...
COMMENT ON FUNCTION public.get_levels_for_proficiency IS 'Returns levels filtered by student proficiency level with recommendation flags (with qualified table names)';
