-- ============================================
-- Safe Lesson Sessions and Grading System Migration
-- ============================================
...
DO $$ BEGIN
  ...
  CREATE INDEX IF NOT EXISTS idx_lesson_sessions_class ON public.lesson_sessions(class);
  CREATE INDEX IF NOT EXISTS idx_lesson_sessions_date ON public.lesson_sessions(date);
  CREATE INDEX IF NOT EXISTS idx_lesson_sessions_status ON public.lesson_sessions(status);
  CREATE INDEX IF NOT EXISTS idx_lesson_sessions_class_date ON public.lesson_sessions(class, date);
END $$;
...
DO $$ BEGIN
  CREATE INDEX IF NOT EXISTS idx_lesson_grades_session ON public.lesson_grades(lesson_session_id);
  CREATE INDEX IF NOT EXISTS idx_lesson_grades_student ON public.lesson_grades(student_id);
  CREATE INDEX IF NOT EXISTS idx_lesson_grades_session_student ON public.lesson_grades(lesson_session_id, student_id);
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    ADD COLUMN lesson_session_id UUID REFERENCES public.lesson_sessions(id) ON DELETE SET NULL;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'update_lesson_sessions_updated_at'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'update_lesson_grades_updated_at'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'auto_assign_lesson_to_submission'
  ) THEN
