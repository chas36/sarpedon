        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {levels.map(level => (
            ...
            >
              ...
              <div className="mb-4">
                {level.moderation_status === 'pending_review' && (
                  <div className="text-xs px-3 py-2 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-yellow-600">
                    ⏳ На модерации
                  </div>
                )}
                {level.moderation_status === 'approved' && (
                  <div className="text-xs px-3 py-2 rounded-lg bg-green-500/10 border border-green-500/20 text-green-600">
                    ✅ Одобрено
                  </div>
                )}
                {level.moderation_status === 'rejected' && (
                  <div className="space-y-2">
                    <div className="text-xs px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/20 text-red-600">
                      ❌ Отклонено
                    </div>
                    {level.moderation_notes && (
                      <div className="text-xs px-3 py-2 rounded-lg bg-learning-bg border border-learning-muted/20 text-learning-muted">
                        <div className="font-medium text-learning-text mb-1">Комментарий модератора:</div>
                        {level.moderation_notes}
                      </div>
                    )}
                  </div>
                )}
                {level.moderation_status === 'draft' && (
                  <div className="text-xs px-3 py-2 rounded-lg bg-learning-muted/10 border border-learning-muted/20 text-learning-muted">
                    📝 Черновик
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center gap-2">
                {level.moderation_status !== 'approved' ? (
                  <>
                    ...
                    {(level.moderation_status === 'draft' || level.moderation_status === 'rejected') && (
                      <Button
                        variant="ghost"
                        size="sm"
                  ...
                  <div className="text-sm text-learning-muted italic w-full text-center">
                    Одобренное задание редактируется только учителем
                  </div>
                )}
              </div>
            </div>
